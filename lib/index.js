...
const requests = require('./foods/requests')
const filter = require('./foods/filter')

$(document).ready(() => {
  requests.getFoods();
  $('.foods').on('click', '.delete-food-btn', requests.deleteFood);
  $('.foods').on('blur', '.food', requests.editFood);
  $('#submit').on('click', requests.addNewFood);
  $('#foodFilter').on('keyup', filter.filterFoods);
});
...
// $('.new-food-btn').on('click', redirectNewFood)
// $('#diaryFilter').on('keyup', filterDiary);
// $('.add-meal-btn').on('click', getCheckBoxIds);
// $('.breakfasts').on('click', '.delete-meal-btn', deleteBreakfastMeal);
