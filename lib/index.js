...
function deleteMeal(event) {
  ...
  return fetch(`https://evening-peak-25213.herokuapp.com/api/v1/meals/` + mealId + `/foods/` + foodId, {
    ...
    .then(removeFromMeal(foodId, mealName))
    .then(updateRemainingCalories(food, mealName))
    .then(updateTotalCalories(food, mealName))
    .then(updateTotalsTable(food, mealName))
    .catch(error => console.log({ error }))
}
...
$('.all-meals').on('click', '.delete-meal-btn', deleteMeal);
